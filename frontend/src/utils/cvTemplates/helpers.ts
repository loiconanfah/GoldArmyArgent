...
/** Safely converts a language entry (string or object) to a display string */
...
/** Safely converts a certification entry (string or object) to a display string */
function certToString(c: any): string {
  if (typeof c === 'string') return c;
  if (typeof c === 'object' && c !== null) {
    return c.name || c.title || c.certification || String(c);
  }
  return String(c ?? '');
}
...
export function extractCvFields(cvData: CvData, parsedAudit: ParsedAudit | null) {
  ...
  const languages     = Array.isArray(cvData.languages) ? cvData.languages.map(langToString) : [];
  const certifications = Array.isArray(cvData.certifications) ? cvData.certifications.map(certToString) : [];

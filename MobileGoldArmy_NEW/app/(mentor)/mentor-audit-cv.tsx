...
export default function MentorAuditCvScreen() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const firstName = (cvData.firstName || parsedAudit?.candidate_name?.split(' ')[0] || 'PRENOM').toUpperCase();
      const lastName = (cvData.lastName || parsedAudit?.candidate_name?.split(' ').slice(1).join(' ') || 'NOM').toUpperCase();
